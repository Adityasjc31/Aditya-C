...
void sortChar(char *sort_ar, int *rest1, int *rest2,int *rest3, int size)
{
    int x, y, index, temp;
    char temp1;
...
void priorty(char *pid, int *at, int *bt, int *pr,int process)
{
    int x, y, burst = 0;
    printf("Gantt chart : \n");
    for (x = 0; x < process; x++)
    {
        ...
        tt[(int)(pid[x] - 65)] = burst-at[x];
        sortIndex(pr,x+1,burst>=process ? process : burst,pid,at,bt);
    }

    sortChar(pid, at, pr, bt, process);
    for (x = 0; x < process; x++)
    {
        wt[x] = tt[x]-bt[x];
    }
...
void main()
{
    ...
    sort(at, pid, bt,pr, process);
    priorty(pid, at, bt,pr, process);
